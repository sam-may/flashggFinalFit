...
parser.add_argument("--exp_only", help = "only plot exp results", action="store_true")
...
for coupling in couplings:
    ...
    for cat in cats_full:
        #if coupling == "Hut" and cat == "FCNCLeptonicTag_2":
        #    continue
        ...
        with open (results[coupling][cat]["input"], "r") as f_in:
            ...
            for line in lines:
                if not ("Expected" in line or "Observed" in line):
                    continue
                else:
                    if "Observed" in line: results[coupling][cat]["obs"] = float(line.split()[-1]) * assumed_br
...
ylabel_bf = { "Hut" : "BF (t #rightarrow Hu) (%)", "Hct" : "BF (t #rightarrow Hc) (%)" }
ylabel_coupling = { "Hut" : "#kappa_{Hut}", "Hct" : "#kappa_{Hct}" }

gamma_t = 1.32158
gamma_hqt = 0.1904
def limit_to_coupling(limit):
    return math.sqrt((limit/100.) * (gamma_t / gamma_hqt)) # factor of 100 since limits are in %

results_bf = copy.deepcopy(results) 
results_coupling = copy.deepcopy(results) 

for coupling, info in results_coupling.items():
    for cat, cat_info in info.items():
        for limit, val in cat_info.items():
            if limit == "input":
                continue
            results_coupling[coupling][cat][limit] = limit_to_coupling(val) 

for coupling in couplings:
    for plot_type, results in zip(["bf", "coupling"], [results_bf, results_coupling]):
        c1 = ROOT.TCanvas("c1", "c1", 800, 800)

        nBins = len(cats_full)
        h_obs = ROOT.TH1F("h_obs", "h_obs", nBins, 0, nBins)
        ...
        if plot_type == "bf":
            ...
            ylabel = ylabel_coupling
            results = results_coupling
        ...
        for cat in cats_full:
            ...
            if not args.exp_only:
                h_obs.SetBinContent(idx, results[coupling][cat]["obs"])
                h_obs.SetBinError(idx, 0.000000001)
        ...
        if plot_type == "bf": 
            ...
            h_exp.SetMinimum(0.03)
            h_exp.SetMaximum(0.3)

        if args.exp_only:
            h_obs = h_exp
        h_obs.Draw("SAME,E,TEXT0")
        ...
        c1.SetLogy()
        c1.SaveAs("limits_%s_%s%s_%s.pdf" % (coupling, args.tag, "_statonly" if args.stat_only else "", plot_type))
        c1.SaveAs("limits_%s_%s%s_%s.png" % (coupling, args.tag, "_statonly" if args.stat_only else "", plot_type))
