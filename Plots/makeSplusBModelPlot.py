...
for c in h_data:
  ...
  canv = ROOT.TCanvas("canv_%s"%c,"canv_%s"%c,1000,1000)
  ...
  if opt.unblind:
    ...
    leg.AddEntry(h_spdfs[c]['pdfNBins'],"FCNC S model","l")
  leg.Draw("Same")
  # Set pdf style
  if opt.unblind:
  ...
  if(opt.loadSnapshot is not None): 
    ...
    for kv in opt.parameterMap.split(","):
      ...
      if "fcnc" in kv:
        assumed_br = 0.1441
        quark = "c" if "ct" in kv else "u"
        poiStr += "BR(t #rightarrow H%s) = %.3f%%%%" % (quark, assumed_br * float(v)) 
      else:
