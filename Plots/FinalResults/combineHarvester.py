...
def writePostamble(sub_file, exec_line):
  ...
  if not opts.dryRun and opts.queue:
    ...
    if (opts.batch == "IC") : system('qsub -q %s -o %s.log -e %s.err %s -l h_rt=720 > out.txt'%(opts.queue,os.path.abspath(sub_file.name),os.path.abspath(sub_file.name),os.path.abspath(sub_file.name)))
    elif( opts.batch == "HTCONDOR" ):
      # FIXME: extract from options
      queue = 'workday'
...
def writeMultiDimFit(method=None,wsOnly=False):
    ...
    "RDiffXsScan"  : "--floatOtherPOIs=1 -P %s"%(opts.poix), # need to add option to run specific topologic categories
        ...
        #FIXME want to omit this, leave the ranges up to the catsMap
