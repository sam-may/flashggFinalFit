...
parser.add_argument("--inputWSDir", help = "directories with input ws", type=str, default = "/home/users/sjmay/ttH/FCNC_Workspaces/CMSSW_10_6_1_patch2/src/flashgg/Systematics/test/workspaces_COUPLING_YEAR_v4.4_17-July-2020/")
...
for coupling in couplings:
    ...
    command_list.append("python makeDatacard.py --tth_unc %.6f --tth_scale %.6f --procs 'tth,ggh,wh,zh,thq,thw,fcnc_%s' --inputWSDir %s --cats %s --years %s --mergeYears --removeNoTag --doSystematics --modelWSDir %s --output %s" % (args.tth_unc, args.tth_scale, coupling.lower(), "fcnc" + "_" + coupling + "_workspaces", args.cats, args.years, "Models_" + coupling, 'Datacard_' + coupling + "_" + args.tag + '.txt'))
    command_list2.append("python convert_smhiggs_tobkg.py --input %s" % 'Datacard_' + coupling + "_" + args.tag + '.txt')
    for cat in args.cats.split(","):
        command_list.append("python makeDatacard.py --tth_unc %.6f --tth_scale %.6f --procs 'tth,ggh,wh,zh,thq,thw,fcnc_%s' --inputWSDir %s --cats %s --years %s --mergeYears --removeNoTag --doSystematics --modelWSDir %s --output %s" % (args.tth_unc, args.tth_scale, coupling.lower(), "fcnc" + "_" + coupling + "_workspaces", cat, args.years, "Models_" + coupling, 'Datacard_' + coupling + "_" + cat + "_" + args.tag + '.txt'))
        command_list2.append("python convert_smhiggs_tobkg.py --input %s" % ('Datacard_' + coupling + "_" + cat + "_" + args.tag + '.txt'))
