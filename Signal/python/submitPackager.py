...
parser.add_option("--basepath",help="path to be prepended to signal files")
parser.add_option("-L","--mhLow",default="120",help="mh Low")
parser.add_option("-H","--mhHigh",default="130",help="mh High")
...
system('mkdir -p %s/PackagerJobs/outputs'%opts.outDir)
...
for cat in opts.flashggCats.split(","):
  ...
  for infile in opts.infile.split(','):
    if not infile.endswith('%s.root'%cat): continue
    specificFiles += '%s,'%infile
  specificFiles = specificFiles[:-1] #remove trailing comma
